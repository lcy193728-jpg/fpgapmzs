...
//          (期望值以下方 EX_* 常量为准, 均由算式推得而非硬抄)
//          ★2026-09-21: 4 张卡的 1× 副标题(小字)已下线 —— 其原占像素现
//            全部归入"卡片内衬", 故 EX_CARD 由 142627 增至 144707,
//            EX_TAG 由 2080 归零(该断言反过来验证副标题确实没有再画出)
...
module tb_osd_menu;
    ...
    //     对应的墨点(gen 打印):
    //     TITLE=2616(1:1)  CT0..3=1129/888/1122/1114
    //     (原 TG0..3=532/499/513/536 为已下线的卡副标题)
    //     MARQ=1295(带内一周期 = 320px)
    //   白色 = 2616 + 4 卡标题(1129+888+1122+1114=4253) = 6869
    //   副标题 = 0(★2026-09-21 下线, 原 2080 墨点像素并入卡内衬);
    //   滚动橙 = 1295×2(整行含两周期) = 2590
    ...
    //     + 430..437行5120) = 117128
    //   卡片内衬 = 4×(40000-640-2120) − 4253 − 0 = 144707
    //   校验和: 117128+144707+8480+2560+6869+0+2590+576+24290 = 307200
    //================================================================
    localparam TOT_PIX   = H_ACT * V_ACT;              // 307200
    localparam EMERG_PIX = 52 * H_ACT;                 // 33280
    localparam EX_BGFULL = 117128;
    localparam EX_CARD   = 144707;   // ★2026-09-21 副标题下线, 由 142627 并入 2080
    localparam EX_BD     = 8480;
    localparam EX_ACC    = 640;
    localparam EX_TXTW   = 6869;
    localparam EX_TAG    = 0;        // ★副标题已下线, 全帧不应再出现该色
    ...
    task check_spots; begin
        if (valid_cnt == 2) begin
            ...
            // ---- ★2026-09-21: 卡副标题(小字)已下线, 这 4 行整行应为纯"卡内衬" ----
            chk(101,132, C_CARD, "TG0 区已清空");
            chk(117,222, C_CARD, "TG1 区已清空");
            chk(104,312, C_CARD, "TG2 区已清空");
            chk(106,402, C_CARD, "TG3 区已清空");
            // ---- 矢量几何特征点 ----
