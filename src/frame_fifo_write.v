...
(
	...
	parameter IMG_WIDTH              = 640,   // 图像宽度(像素)，写地址按行序翻转用
	parameter IMG_HEIGHT             = 480    // 图像高度(行数)，写地址按行序翻转用
)               
(
	input                            rst,                  
		...
		if(state == S_ACK)
			begin
				...
				if(write_addr_index_d1 == 2'd0)
					App_wr_addr_r <= write_addr_0 + ((IMG_HEIGHT-1)*IMG_WIDTH);
				else if(write_addr_index_d1 == 2'd1)
					App_wr_addr_r <= write_addr_1 + ((IMG_HEIGHT-1)*IMG_WIDTH);
				else if(write_addr_index_d1 == 2'd2)
					App_wr_addr_r <= write_addr_2 + ((IMG_HEIGHT-1)*IMG_WIDTH);
				else if(write_addr_index_d1 == 2'd3)
					App_wr_addr_r <= write_addr_3 + ((IMG_HEIGHT-1)*IMG_WIDTH);
			end
		else if(App_wr_en)
			begin
				if(wr_col == IMG_WIDTH-1)
					begin
						wr_col <= 10'd0;
						App_wr_addr_r <= App_wr_addr_r - ((IMG_WIDTH*2) - 1);
					end
				else
					begin
						wr_col <= wr_col + 10'd1;
