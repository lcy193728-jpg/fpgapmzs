...
| 会议(scene_id=1) | 无入场旋律、无背景音乐。当前议题剩余60 s时播放一次提醒音，归零时播放两次超时音；提示由议程状态机事件触发 | 不显示 |
...
4. 会议中KEY1开始/暂停/继续、KEY2下一项、KEY3上一项、KEY4当前项重新计时；数码管显示MM.SS。剩余60 s一声提醒，归零时双声超时；暂停期间不重复触发。
...
- 会议提醒直接使用 `meeting_ctrl` 的 `warn_event/timeout_event`，与当前议题剩余时间同步；
  暂停、继续、切换议题和重新计时均沿用同一状态机的一次性触发规则。
- 可视化缓存与会议议程 BRAM 使全工程使用 60/64 个 BRAM（93.75%）；DSP 使用 11/29（37.93%），
