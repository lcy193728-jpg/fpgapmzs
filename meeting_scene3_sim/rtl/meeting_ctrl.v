...
always @(posedge clk) begin
 ...
 warned<=0;switch_idle<=1;resume_load<=0;end
 else begin
 ...
 state<=MEET_IDLE;alarm_paused<=0;warned<=0;resume_load<=0;end
 ...
 else begin current<=current+1;state<=MEET_SWITCH;switch_idle<=0;end
 end else if(press[2]) begin
 if(current!=0) begin current<=current-1;state<=MEET_SWITCH;switch_idle<=0;end
 end else if(press[3]) begin state<=MEET_SWITCH;switch_idle<=state==MEET_IDLE;end
 ...
 if(state==MEET_IDLE) begin state<=MEET_SWITCH;switch_idle<=0;end
 ...
 if(resume_load || (remaining==0 && overtime==0 && !warned)) begin state<=MEET_SWITCH;switch_idle<=0;end
