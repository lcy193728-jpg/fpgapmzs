...
本版完成公共 PCM→HDMI 输出及固定测试音接入，默认持续播放左右相同的 1 kHz
正弦音，48 kHz、16 位有符号 PCM、峰值 4096（约 -18 dBFS）、启动渐变约 10.7 ms。
没有执行 ModelSim，没有宣称实机声音已经通过。实机验收由队友完成。
...
集成版测试音在菜单及所有场景连续播放，这是底层验收行为；
本版尚未实现各场景不同音频、应急抢占、语音、WAV 播放或音量按键。
KEY1/2/3、KEY4 和 SW 约定不改变。
...
5. SW4：应急画面；解除后恢复原场景。
6. 全程测试音应持续。此版应急时仍是测试音，尚未替换为告警音。
